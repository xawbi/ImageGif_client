...
const LayoutsInProfile: FC<LayoutProps> = ({ avatar, user }) => {
  ...
  return (
    <>
      ...
      <div
        className={`flex justify-between p-6 pb-4 px-2 lg:px-10 sm:px-6 rounded-sm border-[#202330] ${bgProfileClient} bg-cover bg-no-repeat`}>
        <Avatars avatar={avatar} user={user} />
        <div className="flex flex-col items-end">
          <BgProfile bgProfileId={bgProfileId} setBgProfileClient={setBgProfileClient}
                     bgProfileClient={bgProfileClient} />
          <Logout />
        </div>
      </div>
