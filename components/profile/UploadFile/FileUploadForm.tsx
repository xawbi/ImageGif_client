...
const FileUploadForm: FC<ChooseFileBtnProps> = ({handleCloseUploadForm}) => {
  ...
  return (
    <form encType="multipart/form-data" onSubmit={uploadFile} className="w-full flex flex-col items-center">
      ...
      {dragVisible ?
        <>
          {drag ? <div className='py-16 w-54 text-center border-dashed border-2 border-fuchsia-600'
            ...
            <div className='py-16 w-54 text-center border-dashed border-2 border-blue-600'
                   onDragStart={e => dragStartHandler(e)}
                   onDragOver={e => dragStartHandler(e)}
                   onDragLeave={e => dragLeaveHandler(e)}
            >
      ...
      : <button disabled={uploading && true} type='submit' className={`text-blue-500 ${uploading && 'text-gray-500'} mt-4 hover:bg-[#202333] py-2 px-3 c border-2 border-gray-500 lg:text-base rounded-2xl text-sm`}>
